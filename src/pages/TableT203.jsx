...
export default function TableT203() {
  ...
  const [selectedSeatsForItems, setSelectedSeatsForItems] = useState({});

  // T-203 table seats data - 4 seats with regular task flow
  ...
  const filteredSeats = tableSeats.filter(seat => 
  ...
  // Special updateQuantity function for T-203 with seat selection
  const updateQuantity = (itemId, change, selectedSeat = null) => {
    if (expandedCard && expandedCard.currentTask.name === "Order" && selectedSeat) {
      // For Order task, use seat-specific quantities
      const key = `${itemId}_${selectedSeat}`;
      setQuantities(prev => ({
        ...prev,
        [key]: Math.max(0, (prev[key] || 0) + change)
      }));
    } else {
      // For other tasks, use regular quantities
      setQuantities(prev => ({
        ...prev,
  ...
  const getCartItems = () => {
    return Object.entries(quantities)
      .filter(([_, qty]) => qty > 0)
      .map(([itemId, qty]) => {
        const [item, seat] = itemId.includes('_') ? itemId.split('_') : [itemId, null];
        return { 
          id: itemId, 
          name: `Item ${item}`, 
          quantity: qty, 
  ...
  const getAllOrderedItems = () => {
    return getCartItems().map(item => 
      `${item.name} x${item.quantity}${item.seat ? ` (${item.seat})` : ''}`
    ).join(", ");
  };

  // Enhanced color scheme for table seats
          ...
          <div className="px-3 md:px-4 py-3">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl font-bold text-gray-800">Table T-203 Management</h1>
              </div>
            ...
            {/* Table Seats Grid - 2x2 for 4 seats */}
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Table T-203 Management</p>
                </div>
              </div>
                    ...
                    <div key={item} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => updateQuantity(item, -1, selectedSeatsForItems[item])}
                            className="p-1 hover:bg-gray-100 rounded"
                          >
                            <Minus className="w-4 h-4" />
                          </button>
                          <span className="w-8 text-center">
                            {expandedCard.currentTask.name === "Order" && selectedSeatsForItems[item] 
                              ? quantities[`${item}_${selectedSeatsForItems[item]}`] || 0
                              : quantities[item] || 0
                            }
                          </span>
                          <button
                ...
                {getCartItems().map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      {item.seat && <span className="text-xs text-blue-600 ml-2">({item.seat})</span>}
                    </div>
